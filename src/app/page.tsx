...
export default function Page() {
  return (
    <main className="min-h-dvh flex flex-col gap-14 relative">
          ...
          <div className="gap-2 gap-y-6 flex flex-col md:flex-row justify-between">
            <div className="gap-2 flex flex-col order-2 md:order-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                />
                <PronounceMyName namePronunciationUrl={DATA.namePronunciationUrl} />
              </div>
              <BlurFadeText
                className="text-muted-foreground max-w-[600px] text-sm md:text-sm lg:text-base"
                delay={BLUR_FADE_DELAY}
                text={DATA.description}
              />
            </div>
            <BlurFade delay={BLUR_FADE_DELAY} className="order-1 md:order-2">
              <Avatar className="size-24 md:size-32 border rounded-none shadow-lg ring-4 ring-muted">
          ...
          <div className="flex flex-wrap gap-2">
            {DATA.skills.map((skill, id) => (
              <BlurFade key={skill.name} delay={BLUR_FADE_DELAY * 10 + id * 0.05}>
                <div className="border bg-background border-border ring-2 ring-border/20 rounded-none h-8 w-fit px-4 flex items-center gap-2">
      ...
      <section id="contact" className="flex-1 flex flex-col min-h-0">
        <BlurFade delay={BLUR_FADE_DELAY * 16} className="flex-1 flex flex-col min-h-0">
          <ContactSection />
        </BlurFade>
      </section>
    </main>
