...
interface DockIconProps {
  className?: string;
  children?: ReactNode;
  /** When false, icon stays fixed size (no hover magnification). Default true. */
  magnify?: boolean;
}
  ...
  return (
    <DockContext.Provider value={{ mouseX, magnification, distance }}>
      <motion.div
        ...
        className={cn("mx-auto w-max h-full flex items-end justify-center overflow-visible rounded-none border", className)}
      >
        {children}
      </motion.div>
...
const DockIcon = ({ className, children, magnify = true }: DockIconProps) => {
  ...
  if (!magnify) {
    return (
      <div
        ...
        className={cn("relative flex aspect-square items-center justify-center rounded-none shrink-0", className)}
      >
        <div
