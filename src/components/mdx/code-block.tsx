...
export function CodeBlock({ children, ...props }: CodeBlockProps) {
  ...
  return (
    <div className="group relative rounded-none overflow-hidden border border-border">
      <pre
        ref={preRef}
        {...props}
      ...
      >
        {title && (
          <div className="p-3 text-xs font-medium border-b border-border rounded-none bg-muted/50 text-foreground">
            {title}
          </div>
        )}
        ...
        <Button
          ...
          variant="outline"
          size="icon"
          className={cn("absolute size-8 text-primary cursor-pointer right-3 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity rounded-none border border-border shadow-none", title ? "top-13" : "top-3", props.className)}
